...
impl FileUploader {
                    ...
                    if size == 0 {
                        while file_size != bytes_acknowledged {
                            match stream.read_exact(&mut u64_buf) {
                                ...
                                Err(err) => eprintln!("WARNING: failed to read acknowledgement: {}", err)
                            }
                        }
                        break;
                    }
                ...
                Err(e) => {
                    match e.kind() {
                        ErrorKind::ConnectionReset|ErrorKind::BrokenPipe => {
                            ...
                            stream = self.connect();
                            file.seek(SeekFrom::Start(bytes_acknowledged)).expect("Failed to seek");
                            self.send_header(
                                &mut stream, &filename, file_size, bytes_acknowledged);
                        },
                        _ => {
    ...
    fn send_header(
            ...
            filename: &str, file_size: u64, file_offset: u64) {
        ...
        stream.write(&file_size.to_be_bytes())
            .expect("Failed to send file size");

        stream.write(&file_offset.to_be_bytes())
